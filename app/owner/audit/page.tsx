...
export default async function AuditPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <Link href="/owner/system" className="text-sm font-medium text-muted">← System</Link>
        <h1 className="mt-1 text-2xl font-bold text-primary-dark">Audit log</h1>
        <p className="text-sm text-muted">Every money-, contract-, identity- and permission-affecting action.</p>
      </div>

      {rows.length === 0 ? (
        <p className="rounded-[--radius-card] border border-border bg-white p-6 text-center text-muted">No audit entries yet.</p>
      ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-border">
                  <td className="px-3 py-2 text-muted">{r.created_at.slice(0, 16).replace('T', ' ')}</td>
